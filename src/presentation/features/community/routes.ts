...
export class CommunityRoutes {
  static get router (): Router {
    ...
    router.post('/',
      [
        ...
        body(PARAMS_BODY.direccion)
          .notEmpty().withMessage(validationMessages.required(PARAMS_BODY.direccion)),

        body('id_parroquia')
          .notEmpty().withMessage(validationMessages.required('id_parroquia'))
          .isInt().withMessage(validationMessages.notInteger('id_parroquia')),

        body(PARAMS_BODY.telefono)
          .optional()
          .matches(/^\d{8}$/).withMessage('The phone number must have exactly 8 digits'),
    ...
    router.delete('/:id', [param(PARAMS_BODY.id).isInt().withMessage(validationMessages.notInteger('Id_comunidad')), ValidateFields.validate], controller.deleteCommunity)
