    ...
    router.post(
      ...
      [
        body(PARAMS_BODY.nombre)
          ...
          .isString()
          .withMessage(validationMessages.notString(PARAMS_BODY.nombre))
          .isLength({ max: 100 })
          .withMessage(validationMessages.tooLong(PARAMS_BODY.nombre, 100)),
        body(PARAMS_BODY.apellido)
          .notEmpty()
          .withMessage(validationMessages.required(PARAMS_BODY.apellido))
          .isString()
          .withMessage(validationMessages.notString(PARAMS_BODY.apellido))
          .isLength({ max: 100 })
          .withMessage(validationMessages.tooLong(PARAMS_BODY.apellido, 100)),
        body(PARAMS_BODY.telefono)
          .optional({ checkFalsy: true })
          .isString()
          .withMessage(validationMessages.notString(PARAMS_BODY.telefono))
          .isLength({ max: 8, min: 8 })
          .withMessage(validationMessages.tooLong(PARAMS_BODY.telefono, 8)),
        body(PARAMS_BODY.email)
          .optional({ checkFalsy: true })
          .isEmail()
          .withMessage(validationMessages.invalidEmail(PARAMS_BODY.email))
          .isLength({ max: 255 })
          .withMessage(validationMessages.tooLong(PARAMS_BODY.email, 255)),
        ValidateFields.validate,
      ],
      controller.create
    )
