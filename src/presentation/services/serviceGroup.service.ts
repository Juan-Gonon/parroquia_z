...
export class GrupoServicioService {
  ...
  public async getAllGroups(paginationDto: PaginationDto): Promise<any> {
    ...
    try {
      ...
      return {
        ...
        next:
          page * limit < total
            ? `/api/service-group?page=${page + 1}&limit=${limit}`
            : null,
        prev:
          page - 1 > 0
            ? `/api/service-group?page=${page - 1}&limit=${limit}`
  ...
  public async createGroup(
    ...
    if (groupExists) {
      throw CustomError.badRequest(
        'A GrupoServicio with that name already exists.'
      )
    }
    ...
    try {
      ...
      if (error instanceof CustomError) throw error
      throw CustomError.internalServer(
        'An error occurred while creating the GrupoServicio.'
      )
    }
  }
  ...
  public async updateGroup(
    ...
    if (!groupExists) {
      throw CustomError.badRequest(
        'The requested GrupoServicio to update was not found.'
      )
    }

    // Validar nombre único
    if (updateGrupoServicioDto.nombre) {
      const groupNameExists = await this.getGroupByName(
        updateGrupoServicioDto.nombre
      )
      if (
        groupNameExists &&
        groupNameExists.id_grupo !== updateGrupoServicioDto.id
      ) {
        throw CustomError.badRequest(
          'A GrupoServicio with that name already exists.'
        )
    ...
    try {
      ...
      if (error instanceof CustomError) throw error
      throw CustomError.internalServer(
        'An error occurred while updating the GrupoServicio.'
      )
    }
  }
  ...
  public async deleteGroup(id: number): Promise<any> {
    ...
    if (!groupExists) {
      throw CustomError.badRequest(
        'The requested GrupoServicio to delete was not found.'
      )
    }
    try {
      ...
      if (error instanceof CustomError) throw error
      throw CustomError.internalServer(
        'An error occurred while deleting the GrupoServicio.'
      )
    }
  }
}
