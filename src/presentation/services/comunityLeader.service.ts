...
export class LiderComunitarioService {
  ...
  public async getAllLideres (paginationDto: PaginationDto): Promise<any> {
    ...
    try {
      ...
      return {
        ...
        limit,
        total,
        next: (page * limit) < total ? `/api/comunity-leader?page=${page + 1}&limit=${limit}` : null,
        prev: (page - 1 > 0) ? `/api/comunity-leader?page=${page - 1}&limit=${limit}` : null,
        data: lideres
      }
    } catch (error) {
  ...
  public async createLider (createLiderDto: CreateLiderComunitarioDto): Promise<any> {
    ...
    if (liderExists) {
      throw CustomError.badRequest('A community leader with that personal ID and community ID already exists.')
    }
    try {
      ...
      throw CustomError.internalServer('An error occurred while creating the community leader.')
    }
  }

  // Actualizar un Líder
  public async updateLider (updateLiderDto: UpdateLiderComunitarioDto): Promise<any> {
    const liderExists = await this.getLiderById(updateLiderDto.id)
    if (!liderExists) {
      throw CustomError.badRequest('The requested leader to update was not found.')
    }

    try {
      const fechaIni = updateLiderDto.fechaIni ? updateLiderDto.fechaIni : liderExists.fecha_ini
      ...
      throw CustomError.internalServer('An error occurred while updating the community leader.')
    }
  }
  ...
  public async deleteLider (id: number): Promise<any> {
    ...
    if (!liderExists) {
      throw CustomError.badRequest('The requested leader to delete was not found.')
    }
    try {
      ...
      throw CustomError.internalServer('An error occurred while deleting the community leader.')
    }
  }
}
