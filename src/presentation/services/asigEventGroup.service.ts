...
export class AsigGrupoEventoService {
  ...
  public async getAllAsignaciones (pagination: PaginationDto): Promise<any> {
    ...
    try {
      ...
      return {
        ...
        limit,
        total,
        next: (page * limit) < total ? `/api/asigeventgroup?page=${page + 1}&limit=${limit}` : null,
        prev:
           (page - 1 > 0)
  ...
  public async getAsignacionById (id: number): Promise<any> {
    const asignacion = await prisma.asiggrupoevento.findUnique({
      where: {
        id_asig_ge: id
      }
    })
  ...
  public async createAsignacion (createDto: CreateAsigGrupoEventoDto): Promise<any> {
    try {
      ...
      throw CustomError.internalServer('Something went wrong creating the asignacion')
    }
  }

  // Actualizar una asignación
  public async updateAsignacion (updateDto: UpdateAsigGrupoEventoDto): Promise<any> {
    const { id } = updateDto
    ...
    try {
      ...
      console.log(error)
      throw CustomError.internalServer('Something went wrong updating the asignacion')
    }
  }
  ...
  public async deleteAsignacion (id: number): Promise<any> {
    ...
    try {
      ...
      throw CustomError.internalServer('Something went wrong deleting the asignacion')
    }
  }
}
