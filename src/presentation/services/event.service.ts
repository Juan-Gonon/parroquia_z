...
export class EventoService {
  ...
  public async createEvent(createEventoDto: CreateEventoDto): Promise<any> {
    ...
    const comunidadExists = await prisma.comunidad.findUnique({
      where: { id_comunidad: createEventoDto.idComunidad },
    })
    if (!comunidadExists) {
      throw CustomError.badRequest('The specified community does not exist.')
    }

    const tipoEventoExists = await prisma.tipoevento.findUnique({
      where: { id_tipo: createEventoDto.idTipoEvento },
    })
    if (!tipoEventoExists) {
    ...
    if (createEventoDto.idCelebrante) {
      const celebranteExists = await prisma.personalparroquial.findUnique({
        where: { id_personal: createEventoDto.idCelebrante },
      })
      if (!celebranteExists) {
      ...
      const newEvent = await prisma.evento.create({
        data: {
          ...
          nombrecelebranteexterno:
            createEventoDto.nombreCelebranteExterno ?? null,
        },
      })
      return newEvent
    } catch (error) {
  ...
  public async updateEvent(updateEventoDto: UpdateEventoDto): Promise<any> {
    const eventExists = await this.getEventById(updateEventoDto.id)
    if (!eventExists) {
      throw CustomError.badRequest(
        'The requested event to update was not found.'
      )
    }
    ...
    if (updateEventoDto.idComunidad) {
      const comunidadExists = await prisma.comunidad.findUnique({
        where: { id_comunidad: updateEventoDto.idComunidad },
      })
      if (!comunidadExists) {
        throw CustomError.badRequest('The specified community does not exist.')
      }
    ...
    if (updateEventoDto.idTipoEvento) {
      const tipoEventoExists = await prisma.tipoevento.findUnique({
        where: { id_tipo: updateEventoDto.idTipoEvento },
      })
      if (!tipoEventoExists) {
        throw CustomError.badRequest('The specified event type does not exist.')
      }
    ...
    if (updateEventoDto.idCelebrante) {
      const celebranteExists = await prisma.personalparroquial.findUnique({
        where: { id_personal: updateEventoDto.idCelebrante },
      })
      if (!celebranteExists) {
        throw CustomError.badRequest('The specified celebrant does not exist.')
      }
  ...
  public async deleteEvent(id: number): Promise<any> {
    ...
    if (!eventExists) {
      throw CustomError.badRequest(
        'The requested event to delete was not found.'
      )
    }
    try {
