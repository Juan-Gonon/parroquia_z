...
export class RolPersonalService {
  ...
  public async getAllRoles(paginationDto: PaginationDto): Promise<any> {
    ...
    try {
      ...
      return {
        ...
        next:
          page * limit < total
            ? `/api/personal-role?page=${page + 1}&limit=${limit}`
            : null,
        prev:
          page - 1 > 0
            ? `/api/personal-role?page=${page - 1}&limit=${limit}`
  ...
  public async createRol(createRolDto: CreateRolPersonalDto): Promise<any> {
    const rolExists = await this.getRolPersonalByName(createRolDto.nombre)
    if (rolExists) {
      throw CustomError.badRequest(
        'A RolPersonal with that name already exists.'
      )
    }
    try {
      ...
      throw CustomError.internalServer(
        'An error occurred while creating the RolPersonal.'
      )
    }
  }

  // Actualizar un RolPersonal
  public async updateRol(updateRolDto: UpdateRolPersonalDto): Promise<any> {
    const rolExists = await this.getRolById(updateRolDto.id)
    if (!rolExists) {
      throw CustomError.badRequest(
        'The requested RolPersonal to update was not found.'
      )
    }
    ...
    try {
      ...
      throw CustomError.internalServer(
        'An error occurred while updating the RolPersonal.'
      )
    }
  }
  ...
  public async deleteRol(id: number): Promise<any> {
    ...
    if (!rolExists) {
      throw CustomError.badRequest(
        'The requested RolPersonal to delete was not found.'
      )
    }
    try {
      ...
      throw CustomError.internalServer(
        'An error occurred while deleting the RolPersonal.'
      )
    }
  }
}
