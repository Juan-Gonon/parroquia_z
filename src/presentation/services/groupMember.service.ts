...
export class MiembroGrupoService {
  ...
  public async getMiembroById(id: number): Promise<any> {
    try {
      const miembro = await prisma.miembrogrupo.findMany({
        where: { id_gruposervicio: id },
        select: {
          id_miembrogrupo: true,
          id_gruposervicio: true,
          id_part_min: true,
          fecha_ini_msia: true,
          activo: true,
          gruposervicio: {
  ...
  public async getAllMiembros(pagination: PaginationDto) {
    ...
    try {
      ...
      return {
        ...
        next:
          page * limit < total
            ? `/api/group-member?page=${page + 1}&limit=${limit}`
            : null,
        prev:
          page - 1 > 0
            ? `/api/ministry?group-member=${page - 1}&limit=${limit}`
            : null,
        data: miembros,
      }
    } catch (error) {
      throw CustomError.internalServer(
        'An error occurred while retrieving the members.'
      )
    }
  }
    ...
    try {
      ...
      if (!partMinExist) {
        throw CustomError.badRequest(
          'The specified participation does not exist.'
        )
      }
  ...
  public async updateMiembro(updateDto: UpdateMiembroGrupoDto): Promise<any> {
    ...
    try {
      ...
      if (!miembroExist) {
        throw CustomError.badRequest(
          'The requested member to update was not found.'
        )
      }
      ...
      // Validar las fechas
      const fechaIniP = updateDto.fechaIni
        ? updateDto.fechaIni
        : miembroExist.fecha_ini_msia
  ...
  public async deleteMiembro(id: number) {
    try {
      ...
      if (!miembroExist) {
        throw CustomError.badRequest(
          'The requested member to delete was not found.'
        )
      }
